...
#internal import 
from Bike_shop.models import Product, Customer, Order, db 
from Bike_shop.forms import ProductForm
...
def shop():
    ...
    shop_stats = {
        ...
        'sales' : sum([order.order_total for order in allorders]),  # looping through allorders list, order is each object and I'm grabbing the order_total & summing them all up
...
def create():
    ...
    if request.method == 'POST' and createform.validate_on_submit():
        ...
        flash(f"You have successfully created a product {name}", category='success')
        return redirect('/')
...
def update(id):
    ...
    # lets grab our specific product we want to update based on the id
    product = Product.query.get(id) # this should only ever bring back 1 item/object (Same as SELECT * FROM product WHERE prod_id = id)
...
# f string here in order to grab/pass the ^variable(id)^ 

                                            #left of = is html, right of = is what it is referencing in the function  
